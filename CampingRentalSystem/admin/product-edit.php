...
if (isset($_POST['form1'])) {
    ...
    if ($valid == 1) {
        ...
        if ($path == '') {
            ...
            $current_photo = '';
            // Check if the current photo exists and store its filename in a variable
            $statement = $pdo->prepare("SELECT p_featured_photo FROM tbl_product WHERE p_id=?");
            $statement->execute(array($_REQUEST['id']));
            $result = $statement->fetch(PDO::FETCH_ASSOC);
            if ($result) {
                $current_photo = $result['p_featured_photo'];
            }

            // Unlink the previous featured photo if it exists
            if ($current_photo != '') {
