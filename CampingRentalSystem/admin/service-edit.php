...
if(isset($_POST['form1'])) {
    ...
    if($valid == 1) {
        ...
        if($valid == 1) {
            if($path == '') {
                $statement = $pdo->prepare("UPDATE tbl_service SET title=?, content=?, location_link=? WHERE id=?");
                $statement->execute(array($_POST['title'],$_POST['content'],$_POST['location_link'],$_REQUEST['id']));
            } else {
                ...
                $statement = $pdo->prepare("UPDATE tbl_service SET title=?, content=?, photo=?, location_link=? WHERE id=?");
                $statement->execute(array($_POST['title'],$_POST['content'],$final_name,$_POST['location_link'],$_REQUEST['id']));
            }
            ...
            <form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
                ...
                <div class="box box-info">
                    <div class="box-body">
                        ...
                        <div class="form-group">
                            <label for="" class="col-sm-2 control-label">Location Link <span>*</span></label>
                            <div class="col-sm-6">
                                <input type="text" autocomplete="off" class="form-control" name="location_link" value="<?php echo $location_link; ?>">
                            </div>
                        </div>
                        <div class="form-group">
